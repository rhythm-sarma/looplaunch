...
// Optimized 3-tap domain blending for high frame rates
float dbn(vec2 p, float s, float seed) {
  float o = s / 2.0;
  float n0 = vn(p, s, seed);
  float n1 = vn(p + vec2(o, o), s, seed + 0.1);
  float n2 = vn(p + vec2(-o, o), s, seed + 0.2);
  return (2.0 * n0 + 1.5 * n1 + 1.5 * n2) / 5.0;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Smooth radial edge fade computed inside shader (eliminates heavy CSS mask-image)
  vec2 uvCenter = vUv - vec2(0.5, 0.45);
  float dist = length(uvCenter * vec2(1.0, 1.3));
  float radialFade = smoothstep(0.6, 0.15, dist);

  fragColor = vec4(outc, a * uOpacity * radialFade);
}
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL initialization failed for Ferrofluid:', err);
      return;
    }
    ...
    const uniforms = {
      ...
      uGlow: { value: glow },
      uOpacity: { value: prefersReducedMotion ? opacity * 0.5 : opacity },
    ...
    try {
      ...
      console.warn('WebGL shader/mesh initialization failed:', err);
      return;
    }
    ...
    // Pause WebGL rendering whenever scrolled out of viewport!
    ...
    const loop = (t: number) => {
      ...
      // Completely skip GPU work if paused or off-screen
